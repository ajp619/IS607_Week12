...
rank.index <- order(rank.score, decreasing=TRUE)

# Assemle this into a data frame
candidates <- data.frame(cbind(lastname, firstname, GPA, BoardScore, HighestMath, MathCredit, rank.score))
row.names(candidates) <- NULL

# Add a ranking column
candidates <- cbind(candidates[rank.index, ], rank=1:dim(candidates[1]))

# Now display ordered by last name
candidates <- candidates[order(candidates$lastname, candidates$firstname),]
...
Seeing that Einstein ranked 72 and Hawking ranked 174 this is probably not the most accurate system. ;-)
